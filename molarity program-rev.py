...
# some of the bases
Conjugate_Base = input(
    "input the conjugate base of mineral acids or strong oxy acids:")
...
# {}{acd}  + {}{base} --> [][salt]  + []h20
print("The balanced reactions is: \n\n {0}{1}  + {2}{3} --> {6}{4}{2}{5}{0} + {7}H2O" .format(
    water[3], Acid, water[4], Base, cation, water[2], water[5], water[6]))  # --> {}{}{}{}   +  H2O"
...
formula_weight_base = water[1] * \
    atomic_weights["OH"] + atomic_weights[Conjugate_Acid]
formula_weight_water = 2*1.008 + 15.999
formula_weight_salt = water[1]*atomic_weights[Conjugate_Base] + \
    water[0]*atomic_weights[Conjugate_Acid]
...
if mol_salt_given_R1 < mol_salt_given_R2:
    ...
    print("{} is the limiting reactant giving {} moles and {} grams of salt". format(
        Acid, mol_salt_given_R1, grams_of_salt))
else:
    grams_of_salt = mol_salt_given_R2*formula_weight_salt
    print("{} is the limiting reactant giving {} moles and {} grams of salt". format(
        Base, mol_salt_given_R2, grams_of_salt))

if mol_water_given_R1 < mol_water_given_R2:
    grams_of_water = mol_water_given_R1*formula_weight_water
    print("{} is the limiting reactant giving {} moles and {} grams of water". format(
        Acid, mol_water_given_R1, grams_of_water))
else:
    grams_of_water = mol_water_given_R2*formula_weight_water
    print("{} is the limiting reactant giving {} moles and {} grams of water". format(
        Base, mol_water_given_R2, grams_of_water))
